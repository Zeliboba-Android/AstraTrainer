...
function parseHeadTailArgs(args) {
  ...
  while (i < args.length) {
    const arg = args[i];
    if (arg === '-n') {
      if (i + 1 >= args.length) return { error: 'Missing value for -n option' };
      const num = parseInt(args[i + 1], 10);
      if (isNaN(num) || num <= 0)
        return { error: `Invalid number of lines: ${args[i + 1]}` };
      lines = num;
      i += 2;
      ...
      if (isNaN(num) || num <= 0)
        return { error: `Invalid number of lines: ${numStr}` };
      lines = num;
      i += 1;
    } else if (arg.startsWith('-')) {
      return { error: `Invalid option: ${arg}` };
    } else {
      if (fileName) return { error: 'Multiple file names specified' };
      fileName = arg;
      i += 1;
    }
  }
...
const commandHelp = {
  ...
  cut: {
    ...
    examples: ["cut -d':' -f1 file.txt", 'cut -f2 data.csv'],
  },
  ifconfig: {
  ...
  grep: {
    ...
    examples: ['grep "error" log.txt', 'grep "TODO" notes.txt'],
  },
  adduser: {
...
const commands = {
  ls: () => {
    ...
    return Object.keys(dir).map(formatFileName).join('  ');
  },
  ...
  mkdir: (args) => {
    ...
    dir[dirName] = {};
    saveState();
    return `Directory created: ${formatFileName(dirName)}`;
  },
  ...
  rm: (args) => {
    ...
    if (dir[target] === undefined) {
      return `File or directory ${formatFileName(target)} not found.`;
    }
    ...
    if (dir[target] === null || typeof dir[target] !== 'object') {
      delete dir[target];
      saveState();
      return `File ${formatFileName(target)} deleted.`;
    }
  ...
  cp: (args) => {
    ...
    if (dir[source] === undefined) {
      return `Source file ${formatFileName(source)} not found.`;
    }

    dir[destination] = dir[source];
    saveState();
    return `Copied ${formatFileName(source)} to ${formatFileName(destination)}.`;
  },
  ...
  mv: (args) => {
    ...
    saveState();
    return `Moved ${formatFileName(source)} to ${formatFileName(destination)}`;
  },
  ...
  touch: (args) => {
    ...
    dir[fileName] = null;
    saveState();
    return `File ${formatFileName(fileName)} created.`;
  },
  ...
  rmdir: (args) => {
    ...
    delete dir[dirName];
    saveState();
    return `Directory ${formatFileName(dirName)} removed.`;
  },
  ...
  cat: (args) => {
    ...
    if (dir[fileName] === undefined || dir[fileName] === null) {
      return `File ${formatFileName(fileName)} not found or is empty.`;
    }
  ...
  echo: (args) => {
    ...
    if (dir[fileName] === undefined) {
      ...
      saveState();
      return `Text written to ${formatFileName(fileName)}`;
    }
  ...
  grep: (args) => {
    ...
    const dir = resolvePath(currentPath);
    if (!dir) return 'Directory not found';
    if (!dir[fileName] || dir[fileName] === null)
      return `File ${formatFileName(fileName)} not found`;
  ...
  head: (args) => {
    const parsed = parseHeadTailArgs(args);
    if (parsed.error) return parsed.error;

    const { lines, fileName } = parsed;
    const dir = resolvePath(currentPath);
    if (!dir) return 'Directory not found.';
    if (dir[fileName] === undefined || dir[fileName] === null)
      return `File ${formatFileName(fileName)} not found.`;
...
// В функции handleCommand заменим строку:
// const [command, ...args] = input.trim().split(/\s+/);
// на новую логику разбора аргументов:
